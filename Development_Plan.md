...
├── message_widget.py           # 单条消息气泡组件（用户/AI/工具卡片）
...
│                                                  │
│  ┌──────────┐  ┌──────────────┐  ┌───────────┐ │
│  │ sidebar   │  │ agent_engine │  │ llm_client│ │
│  │ (UI 层)   │→│ (控制层)      │→│ (API 层)  │ │
│  └──────────┘  └──────┬───────┘  └───────────┘ │
│                       ↓                          │
│              ┌────────────────┐                  │
...
│                       ↓                          │
│  ┌────────────┐ ┌───────────┐ ┌──────────────┐ │
│  │ tools/     │ │ context_  │ │ conversation_│ │
│  │ (10个工具)  │ │ manager   │ │ manager      │ │
│  └────────────┘ └───────────┘ └──────────────┘ │
│  ┌─────────────────┐                            │
│  │ file_source_    │                            │
│  │ manager         │                            │
...
**标题生成：** 用户首条消息后异步调用 LLM 生成（10-20 字），fallback 到首条消息截取
...
| 3 | `llm_client.py` | OpenAI 兼容协议、SSE 流式解析、多 Provider、function calling |
| 4 | `settings_dialog.py` | API Key 输入、Provider/Model 选择、测试连接 |
...
| 步骤 | 文件 | 内容 |
|------|------|------|
| 6 | `sidebar.py` | QDockWidget 主框架、消息列表 QScrollArea、输入框、顶部栏按钮、文件附件区 |
| 7 | `message_widget.py` | 用户消息气泡（右对齐）、AI 消息气泡（左对齐） |
| 8 | `tool_card.py` | 可折叠工具卡片（spinner/成功/失败状态） |
| 9 | — | 流式文本实时追加显示 |
...
| 步骤 | 文件 | 内容 |
|------|------|------|
| 10 | `tool_registry.py` | 3 个核心工具的 JSON Schema 定义 + Python 函数映射 |
| 11 | `context_manager.py` | 项目图层信息收集（名称、类型、CRS、字段、要素数），支持 source 过滤 |
| 12 | `file_source_manager.py` | 文件源管理、临时图层加载/卸载、QGIS 信号监听 |
...
| 步骤 | 文件 | 内容 |
|------|------|------|
| 16 | `agent_engine.py` | 对话循环、tool_calls 解析、工具执行、结果反馈、滑动窗口 |
...
| 步骤 | 文件 | 内容 |
|------|------|------|
| 21 | `conversation_manager.py` | SQLite 存储、对话 CRUD、自动保存、plugin_files 持久化 |
| 22 | `sidebar.py` (更新) | 历史列表视图、搜索、切换、删除 |
